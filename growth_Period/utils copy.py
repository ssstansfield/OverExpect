...
def SUE(stock_code, date:str, num_monitoring:int, reports_dict:dict):
    try:
        ...
        print(stock_code, date, 'not in reports_dict')
        return np.nan
    reports = reports[reports['m_anntime'] <= date]
    reports.drop_duplicates('m_timetag', keep='last', inplace=True)
    ...
    if reports.shape[0] < num_monitoring + 5:
        return np.nan
    current_tag_idx = time_tags.get_loc(current_timetag)
    current_np = reports.loc[current_timetag]['net_profit_incl_min_int_inc']
    last_np = reports.iloc[current_tag_idx - 4]['net_profit_incl_min_int_inc']
    past_np_delta = []
    for i in range(1, num_monitoring + 1):
        past_np_delta.append(reports.iloc[current_tag_idx - i]['net_profit_incl_min_int_inc'] - reports.iloc[current_tag_idx - i - 4]['net_profit_incl_min_int_inc'])
    past_np_delta = np.array(past_np_delta)
    expected_np = last_np + np.mean(past_np_delta)
    SUE = (current_np - expected_np) / np.std(past_np_delta)
    return SUE
